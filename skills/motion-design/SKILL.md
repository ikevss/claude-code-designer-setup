---
name: motion-design
description: 动效设计方法论 | 动画节奏不对、不知道用什么缓动曲线、多个元素动画怎样编排先后顺序、加载动画没灵魂、页面过渡太生硬、把迪士尼动画原则用到UI里、动效的时长/缓动/编排三要素指导 | motion design, animation principles, easing, choreography, Disney principles, loading animation, UI motion — 本技能侧重动效的设计方法论、该不该动怎么动才舒服，不覆盖代码实现
license: MIT
metadata:
  author: LottieFiles
  version: "1.0.0"
