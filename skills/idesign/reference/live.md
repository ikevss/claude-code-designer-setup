...
1. `live.mjs`: boot. If the request names or implies a file, route, or app inside a monorepo, infer the concrete path and run `node .claude/skills/idesign/scripts/live.mjs --target <path>` instead; then run the rest of this live session from the returned `projectRoot`.
...
```bash
node .claude/skills/idesign/scripts/live.mjs
```
...
```
LOOP:
  node .claude/skills/idesign/scripts/live-poll.mjs   # default long timeout; no --timeout=
...
```
node .claude/skills/idesign/scripts/live-poll.mjs --stream   # stays running; one JSON line per event
...
The live helper persists an append-only journal under `.idesign/live/sessions/`. Browser checkpoints are advisory but durable; the journal is canonical. This is local durable recovery state, not project source.

Use these commands when the chat was interrupted, polling was missed, the helper restarted, or the browser reloaded:

```bash
node .claude/skills/idesign/scripts/live-status.mjs
node .claude/skills/idesign/scripts/live-resume.mjs --id SESSION_ID
node .claude/skills/idesign/scripts/live-complete.mjs --id SESSION_ID
```
...
```bash
node .claude/skills/idesign/scripts/live-insert.mjs --id EVENT_ID --count EVENT_COUNT --position after \
...
```bash
node .claude/skills/idesign/scripts/live-wrap.mjs --id EVENT_ID --count EVENT_COUNT --element-id "ELEMENT_ID" --classes "class1,class2" --tag "div" --text "TEXT_SNIPPET"
```
...
```bash
node .claude/skills/idesign/scripts/live-poll.mjs --reply EVENT_ID done --file RELATIVE_PATH
```
...
```bash
node .claude/skills/idesign/scripts/live-poll.mjs --reply EVENT_ID error "Short reason"
```
...
```bash
node .claude/skills/idesign/scripts/live-poll.mjs --reply EVENT_ID steer_done ["Optional short note for a browser toast"]
```
...
When native subagents are available, delegate source edits to `impeccable_manual_edit_applier` / `idesign-manual-edit-applier`. Pass cwd, scripts path, event id, page URL, chunk/deadline, `batch`, `evidencePath`, and the canonical JSON result schema. The subagent must not poll or reply. If unavailable, apply inline with the same contract.
...
```bash
node .claude/skills/idesign/scripts/live-server.mjs stop
```

Stops the HTTP server and runs `live-inject.mjs --remove` to strip `localhost:…/live.js` from the HTML entry. To stop the server but keep the inject tag (for a quick restart), use `stop --keep-inject`. `.idesign/live/config.json` persists as project config for future sessions.
...
If `live.mjs` outputs `{ ok: false, error: "config_missing" | "config_invalid", path }`, write the live config at the reported path. By default this is `.idesign/live/config.json`.
...
```bash
node .claude/skills/idesign/scripts/detect-csp.mjs
```
...
- **`null`**: no CSP; skip to writing `.idesign/live/config.json` with `cspChecked: true`.
...
If a user says "no" to the CSP patch at setup time and later complains that live doesn't work: their dev CSP blocks `http://localhost:8400`. Fix: delete `cspChecked` from `.idesign/live/config.json` and re-run `live.mjs`: setup will ask again.
