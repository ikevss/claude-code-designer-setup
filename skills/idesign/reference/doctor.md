Report and repair drift between this project's Impeccable artifacts and what the installed version reads: PRODUCT.md, DESIGN.md and its `.idesign/design.json` sidecar, `.idesign/config.json`, persisted surface briefs, and the design hook.
...
```
node .claude/skills/idesign/scripts/doctor.mjs --json
```
...
- **`auto`** carries no decision. Run `node .claude/skills/idesign/scripts/doctor.mjs --fix` once to apply these, then report what it moved in one line. Do not ask permission first, and do not ask about them afterward.
