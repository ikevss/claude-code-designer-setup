...
2. **Confirm the target slugs cleanly**:
   ```bash
   node .claude/skills/idesign/scripts/critique-storage.mjs slug "<resolved-path-or-url>"
   ```
...
3. **Read `.idesign/critique/ignore.md`** if it exists. Drop matching findings silently; it is the only prior-run input critique consumes.
...
CLI scan:
```bash
node .claude/skills/idesign/scripts/detect.mjs --json [target]
```
...
4. If mutation is available, start `node .claude/skills/idesign/scripts/live-server.mjs --background`, present the browser if supported, label `[Human]`, scroll top, inject `http://localhost:PORT/detect.js`, wait 2-3 seconds, read `idesign` console messages, then stop the live server.
...
- **Suggested command**: Which command could address this (from: /idesign adapt, /idesign animate, /idesign audit, /idesign bolder, /idesign clarify, /idesign colorize, /idesign critique, /idesign delight, /idesign distill, /idesign document, /idesign harden, /idesign layout, /idesign onboard, /idesign optimize, /idesign overdrive, /idesign polish, /idesign quieter, /idesign shape, /idesign typeset)

#### Persona Red Flags
> *Consult the [Personas reference](#persona-based-design-testing) below.*

Auto-select 2-3 personas most relevant to this interface type (use the selection table in the reference). If `CLAUDE.md` contains a `## Design Context` section from `idesign init`, also generate 1-2 project-specific personas from the audience/brand info.
...
Once the report above is finalized, write it to `.idesign/critique/` so the user can refer back, and so `/idesign polish` can pick up the priority issues without a copy-paste.
...
2. **Pass the structured metadata** through `IMPECCABLE_CRITIQUE_META` (JSON), then run the write command:
   ...
   IMPECCABLE_CRITIQUE_META='{"target":"<user phrasing>","total_score":<n>,"max_score":<n>,"na_heuristics":"<comma-separated numbers, or empty>","p0_count":<n>,"p1_count":<n>}' \
     node .claude/skills/idesign/scripts/critique-storage.mjs write "<resolved target>" <body-file>
   ```
...
4. **Read the trend** for context:
   ```bash
   node .claude/skills/idesign/scripts/critique-storage.mjs trend "<resolved target>" 5
   ```
...
5. **Append a single line to the user-visible output**, after the report and before the questions:
   ...
   > Wrote `.idesign/critique/<filename>`.
...
- Only recommend commands from: /idesign adapt, /idesign animate, /idesign audit, /idesign bolder, /idesign clarify, /idesign colorize, /idesign critique, /idesign delight, /idesign distill, /idesign document, /idesign harden, /idesign layout, /idesign onboard, /idesign optimize, /idesign overdrive, /idesign polish, /idesign quieter, /idesign shape, /idesign typeset
...
- End with `/idesign polish` as the final step if any fixes were recommended

After presenting the summary, tell the user:

> You can ask me to run these one at a time, all at once, or in any order you prefer.
>
> Re-run `/idesign critique` after fixes to see your score improve.
...
If `CLAUDE.md` contains a `## Design Context` section (generated by `idesign init`), derive 1–2 additional personas from the audience and brand information:
