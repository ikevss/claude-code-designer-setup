...
Read this when the user invokes `/idesign` with no argument. They are asking "what should I do?" Make the menu context-aware instead of static.

Setup has already run `context.mjs`. If that reported `NO_PRODUCT_MD`, the project has no captured context yet: lead the menu with `/idesign init` as the top recommendation (one line on why) and still show the rest below; don't silently jump into init. Otherwise run `node .claude/skills/idesign/scripts/context-signals.mjs` once and read its JSON, then lead with the **2-3 highest-value next commands**, each with a one-line reason pulled from the signals, followed by the full menu (the Commands table in SKILL.md, grouped by category). **Never auto-run a command; the recommendation is a suggestion the user confirms.**

**Chinese input routing**: When the user speaks Chinese to describe a design task (e.g. "审查", "配色", "打磨", "做减法"), load [zh-commands.md](zh-commands.md) and match their intent to the correct sub-command. The user doesn't need to know the English command names. If the intent is ambiguous (especially `audit` vs `critique`), ask one clarifying sentence before picking.

Reason over the signals; there is no score to obey:

- `setup.hasDesign` false while `setup.hasCode` true → `document` (capture the visual system).
- `critique.latest` is `null` → the project has never been critiqued; for a set-up project with a real surface, offering `/idesign critique <surface>` is a strong default.
