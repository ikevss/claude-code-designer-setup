...
// behavior with `.idesign/config.json` → `hook: { "perEditRules": "all" }`.
...
// A project opts back in with `.idesign/config.json`:
...
// Where `.idesign/` (cache + config) lives for this event. Normally the
...
export function appendDesignSystemNote(text, scanOptions) {
  ...
  return `${text}\n\n${ENVELOPE_PREFIX} DESIGN.md is newer than .idesign/design.json. Run ${IMPECCABLE_COMMAND} document to refresh the design-system sidecar.`;
}
...
export async function runHook({ stdinJson, env = {}, cwd = process.cwd(), now = Date.now, detector } = {}) {
  ...
  try {
    ...
    // `.idesign/` (dedup and suppression need it), deferred findings do
    // too (the Stop deep pass needs the touched-file list to surface them),
    // and an already-present `.idesign/` dir marks a project that opted
