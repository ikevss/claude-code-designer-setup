...
// DESIGN sidecar is project-local at .idesign/design.json, with legacy
...
function loadBrowserScripts() {
  ...
  const detectPaths = [
    ...
    path.join(__dirname, '..', '..', '..', '..', 'cli', 'engine', 'detect-antipatterns-browser.js'),
    path.join(process.cwd(), 'node_modules', 'idesign', 'cli', 'engine', 'detect-antipatterns-browser.js'),
  ];
  let detectScript = '';
...
function createRequestHandler({ detectScript, liveScriptParts }) {
  return (req, res) => {
    ...
    //   /design-system.json    returns both parsed DESIGN.md and .idesign/design.json
    ...
    //                          - sidecar: .idesign/design.json contents when present.
    ...
    if (p === '/design-system.json' || p === '/design-system/raw') {
      ...
      if (jsonStat) {
        try {
          ...
          response.sidecarError = 'Failed to parse .idesign/design.json: ' + err.message;
        }
      }
...
function sessionFileMetadataFromPollReply(file) {
  ...
  if (!metadataFile.includes('node_modules/.impeccable-live/')
      && !metadataFile.includes('src/lib/idesign/')
