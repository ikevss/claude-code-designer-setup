 ...
 *      `.idesign/config.json` declares `projectRoots`)
...
const UPDATE_HOST = (process.env.IMPECCABLE_UPDATE_HOST || 'https://idesign.style').replace(/\/$/, '');
...
function buildUpdateDirective(localVersion, latestVersion) {
  return (
    `UPDATE_AVAILABLE: A newer idesign skill is available ` +
...
const HOOK_MANIFESTS_BY_PROVIDER = Object.freeze({
  ...
  github: ['.github/hooks/idesign.json'],
  grok: ['.grok/hooks/idesign.json'],
});
...
function valueHasHookMarker(value) {
  if (typeof value === 'string') {
    return value.includes('skills/idesign/scripts/hook.mjs')
      || value.includes('skills/idesign/scripts/hook-before-edit.mjs');
  }
...
function hookEnabledAt(root) {
  ...
  let enabled = true;
  for (const name of ['.idesign/config.json', '.idesign/config.local.json']) {
