 ...
 * .idesign/config.json / .idesign/config.local.json.
 *
 * Usage:
...
const IMPECCABLE_HOOK_COMMAND_MARKERS = [
  'skills/idesign/scripts/hook-probe.mjs',
  'skills/idesign/scripts/hook.mjs',
  'skills/idesign/scripts/hook-before-edit.mjs',
  'skills/idesign/scripts/hook-after-edit.mjs',
  'skills/idesign/scripts/hook-stop.mjs',
];
...
const HOOK_MANIFEST_TARGETS = [
  {
    ...
    manifest: () => ({
      description: 'idesign design detector: immediate-tier checks after Edit/Write/MultiEdit on UI files, full-rule deep pass on Stop.',
      hooks: {
        PostToolUse: [
          {
            ...
            hooks: [
              {
                type: 'command',
                command: 'node "${CLAUDE_PROJECT_DIR}/.claude/skills/idesign/scripts/hook.mjs"',
        ...
        Stop: [stopManifestEntry('node "${CLAUDE_PROJECT_DIR}/.claude/skills/idesign/scripts/hook.mjs"')],
      },
    }),
  },
  {
    ...
    manifest: () => ({
      hooks: {
        PostToolUse: [
          {
            ...
            hooks: [
              {
                type: 'command',
                command: 'node ".agents/skills/idesign/scripts/hook.mjs"',
                timeout: TIMEOUT_SECONDS,
                statusMessage: STATUS_MESSAGE,
              },
            ],
        ...
        Stop: [stopManifestEntry('node ".agents/skills/idesign/scripts/hook.mjs"')],
      },
    }),
  },
  {
    ...
    manifest: () => ({
      ...
      hooks: {
        preToolUse: [
          {
            command: 'node ".cursor/skills/idesign/scripts/hook-before-edit.mjs"',
            timeout: TIMEOUT_SECONDS,
          },
        ],
      },
  ...
  },
  {
    ...
    destRel: '.github/hooks/idesign.json',
    manifest: () => ({
      version: 1,
      hooks: {
        postToolUse: [
          {
            ...
            bash: 'node "$(git rev-parse --show-toplevel)/.github/skills/idesign/scripts/hook.mjs"',
...
function statusReport(cwd) {
  ...
  const cfgPath = path.relative(cwd, getConfigPath(cwd)) || '.idesign/config.json';
  const localPath = path.relative(cwd, getLocalConfigPath(cwd)) || '.idesign/config.local.json';
  const cachePath = path.relative(cwd, getCachePath(cwd)) || '.idesign/hook.cache.json';
