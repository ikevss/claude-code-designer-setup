---
name: gsap-utils
description: GSAP 工具函数集 gsap.utils — clamp 范围约束、mapRange 值映射、normalize 归一化、interpolate 插值、random 随机、snap 吸附取整、toArray 转数组、wrap 循环、pipe 链式变换、distribute 分布分配、selector 作用域选择器 | 把 0-100 映射到 0-1、限定数值范围、随机数、网格吸附、颜色解析、单位处理、工具函数、utils、helper
license: MIT
---
