...
4. **A4 出图**：必须逐页调用 imagegen / GPT Image API 出图；**每页必须是含全部真实文字的成品图，绝不占位符/空模板**；错字/失败只重出该页；将生成源图复制到 `slides/`；写 `imagegen-manifest.json`（backend 标注 `gpt-image-api` 或 `codex-imagegen`）。
5. **A5 合成**：`compose_pptx.py deck.json out/<topic>-image-deck.pptx`（每页只一个 `background`）。

### A4 阻塞门禁

- “生成图片”只指调用图像生成模型（GPT Image API / Codex imagegen / 原生出图工具），不指用 Python/PIL、SVG、HTML、Canvas、matplotlib、PowerPoint shapes 或截图渲染生成 PNG。
- 禁止先做可编辑 PPTX / 原生 shapes，再导出图片充当阶段 1。
- 禁止用代码在生成图上补字、盖字、改字。
- 若所有后端都不可用，必须停止并说明阻塞原因，不能用代码绘图兜底。
...
- 🔴 阶段 1：**必须调用 imagegen / GPT Image API 生成每页图片**，禁止任何代码绘制整页幻灯片图替代。
- 🔴 阶段 2：**B2 背景、B3 骨架图、B4 元素图标图都必须调用 imagegen / GPT Image API 生成**。禁止用原生 `shapes`、PPT 色块、代码绘图或裁原图局部替代图片层。
