...
1. **用户当条消息点名了后端** → 用它。
2. **GPT Image API（本技能内置）** → 用 `scripts/image_generator.py`（**推荐，跨运行时统一，无需 Codex**）。详见 §7。
3. **Codex（GPT）运行时** → 用内置 `imagegen` skill（需要 Codex 环境）。
4. **其它原生图像工具**（如 Cursor 的 `GenerateImage`、Hermes `image_generate`）→ 用之。
5. 都没有 → 告诉用户并询问如何继续。
...
## 7. GPT Image API（本技能内置，跨运行时统一后端）

本技能已内置 `scripts/image_generator.py`，通过 OpenAI 兼容的 `/v1/images/generations` 端点生图。**这是推荐的首选后端**，无需 Codex 环境，在任何运行时（Claude Code / Cursor / 终端）均可直接使用。

### 配置（一次性）

技能根目录的 `.env` 文件（已 git-ignored）：
...
GPT_IMAGE_MAX_RETRIES=3
```

未配置时复制 `.env.example` → `.env` 并填入凭证。
...
gen = ImageGenerator()
gen.generate(
    "Recreate this slide background WITHOUT any text, icons, cards, or frames. "
    "Keep the original gradient, colors, and lighting exactly.",
    "background.png",
    out_dir="./output"
)
```
...
```python
gen.generate(
    "Extract the frame/skeleton layer on pure #00ff00 green background. "
    "Include: card containers with fills, title bars, divider lines, "
    "chart shapes (line/bar/pie), coordinate grids, ribbons, decorations. "
    "Exclude: all text, icons, decorative icons. "
    "Keep original colors, fills, and 1:1 positioning.",
    "frame_raw.png",
    out_dir="./output"
)
```
...
```python
gen.generate(
    "Place all icons and decorative elements from the source slide "
    "onto a pure #00ff00 green background in a 4x4 grid layout. "
    "Keep original colors and shapes. No dividing lines between cells.",
    "icons_raw_1.png",
    out_dir="./output",
    size="2048x2048"  # 方形图标表
)
```
...
| 参数 | 默认值 | 说明 |
|------|--------|------|
| `GPT_IMAGE_MODEL` | `gpt-image-2` | 模型名（`gpt-image-2-plus` 质量更高） |
| `GPT_IMAGE_SIZE` | `1920x1080` | 背景/框架用 16:9 |
| `GPT_IMAGE_TIMEOUT` | `300` | 单次请求超时（秒） |
| `GPT_IMAGE_MAX_RETRIES` | `3` | 失败重试次数 |

### 比例速查

- **背景/框架** → `1920x1080`（16:9）或 `1920x1280`（3:2）
- **图标表** → `2048x2048`（1:1 正方形，便于网格排列）

### manifest 记录

使用 GPT Image API 出图时，`imagegen-assets-manifest.json` 应记录：
- `backend`: `"gpt-image-api"`
- `generated_source`: API 返回的图片本地路径
- `copied_to`: 复制到 RUN_ROOT 内的最终路径
- `model`: 使用的模型名
- `prompt_file`: 对应的提示词文件
- `layer`: `"background"` / `"frame"` / `"icons"`
