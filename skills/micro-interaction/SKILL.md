...
description: 微交互动效 | 按钮hover/press的触感反馈、开关toggle的过渡动画、点赞按钮的心跳动效、Toast通知滑入滑出、抽屉/弹窗的进出动画、列表项增删的排序动效、共享元素过渡（点击一张图展开到详情页）、UI反馈的细微打磨 | micro-interaction, hover effect, button animation, toast animation, Framer Motion, shared element transition
version: 0.1.0
---
