#!/usr/bin/env python3
"""
GordenImagePPTGen — Core Image Generator Module

Reads configuration from the skill's own .env file, provides a unified
generate_image() function for slide page generation.

Supports both:
  - GPT Image 2 API (OpenAI-compatible /v1/images/generations endpoint)
  - Codex built-in imagegen (via Skill tool)

Usage from other scripts:
    from image_generator import ImageGenerator

    gen = ImageGenerator()  # loads .env automatically
    path = gen.generate("prompt text", "output_filename.png")
"""
...
def _ensure_deps() -> None:
    ...
    print(f"[GordenImagePPTGen] 首次运行，自动安装缺少的依赖: {', '.join(missing)}")
    try:
        subprocess.check_call(
            [sys.executable, "-m", "pip", "install", "--quiet", *missing],
            stdout=subprocess.DEVNULL,
        )
        print(f"[GordenImagePPTGen] 依赖安装完成")
    except subprocess.CalledProcessError as exc:
        raise SystemExit(
            f"[GordenImagePPTGen] 自动安装失败，请手动执行:\n"
...
class ImageGenerator:
    ...
    This class is the primary image generation backend for GordenImagePPTGen.
    It replaces the Codex built-in imagegen when running in non-Codex environments,
    or serves as a fallback when imagegen is unavailable.
    """
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="GordenImagePPTGen — Test image generation")
    parser.add_argument("--prompt", default="A professional slide with dark blue gradient background, 16:9 ratio")
