...
```bash
# 单张出图
python3 scripts/image_generator.py --prompt "提示词" --out slide_01.png
...
# 在 generate_final.py 等同目录脚本中直接 import
from image_generator import ImageGenerator
```
...
| `GPT_IMAGE_SIZE` | `1920x1080` | 16:9 输出尺寸 |
| `GPT_IMAGE_TIMEOUT` | `300` | 单次请求超时（秒） |
| `GPT_IMAGE_MAX_RETRIES` | `3` | 失败重试次数 |

### 比例速查

- **16:9** → `1920x1080`（默认幻灯片比例）
- **3:2** → `1920x1280`
- **1:1** → `1024x1024`（图标提取用）

### generate_final.py（批量出图脚本）

```bash
# 从 slides.json 批量生成
python3 scripts/generate_final.py --slides slides.json --out final_pages/

# 指定锚点文本
python3 scripts/generate_final.py --slides slides.json --anchor "科技商务风格，深蓝配色..." --out final_pages/

# 仅生成指定页
python3 scripts/generate_final.py --slides slides.json --only S01,S05 --out final_pages/
```

### manifest 记录

使用 GPT Image API 出图时，`imagegen-manifest.json` 应记录：
- `backend`: `"gpt-image-api"`
- `generated_source`: API 返回的图片本地路径
- `copied_to`: 复制到 slides/ 的最终路径
- `model`: 使用的模型名
