---
name: svg-animation
description: SVG 动画 | 让线条一笔一笔画出来（描边动画）、签名手写效果、一个形状变成另一个形状（路径形变）、图标/Logo动起来、元素沿路径移动、SVG渐变或滤镜的动画效果 | SVG animation, stroke dashoffset, path morphing, draw-on effect, icon animation, motion along path
version: 0.1.0
---
