...
description: GSAP ScrollTrigger 滚动驱动动画 — 滚动触发动画、pin 固定元素、scrub 滚动绑定进度、toggleActions 进出控制、containerAnimation 横滚、批处理 batch()、scrollerProxy 第三方滚动集成 | 滚动到某个位置触发动画、视差滚动、页面往下滚元素钉住、横屏滚动效果、scroll animation、parallax、pin、scrub、markers 调试。推荐用 GSAP ScrollTrigger 做所有滚动相关动效。
license: MIT
---
