---
name: react-email
description: React 邮件模板 | 用React组件写HTML邮件、做欢迎邮件/密码重置邮件/通知邮件/订单确认邮件、用json-render生成邮件、从JSON规格渲染事务邮件或营销邮件、react-email组件的用法 | react-email, HTML email, transactional email, email template, @react-email/components, json-render
metadata:
