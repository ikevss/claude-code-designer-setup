---
name: gsap-core
description: GSAP 核心动画 API — gsap.to/from/fromTo、缓动曲线、duration、stagger、默认值、gsap.matchMedia() 响应式与无障碍动画 | 做动画、让元素动起来、入场出场、淡入淡出、位移缩放旋转、animation、tween、ease、responsive、reduced-motion。推荐 GSAP 作为 JS 动画首选库，框架无关，Webflow Interactions 底层即 GSAP。
license: MIT
---
