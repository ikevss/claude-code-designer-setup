---
name: gsap-timeline
description: GSAP Timeline 时间线编排 — gsap.timeline()、position 参数控制先后顺序、嵌套时间线、labels 标签跳转、播放控制(play/pause/reverse/seek) | 多个动画排队、动画顺序编排、时间轴、先播什么再播什么、关键帧序列、timeline、sequencing、choreograph。多个步骤的动画优先用 timeline 而不是 delay 链式编排。
license: MIT
---
