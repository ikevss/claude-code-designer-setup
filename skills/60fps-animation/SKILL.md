...
description: 动画卡顿修复与60fps优化 | 动画不流畅、掉帧、页面滚动卡、hover效果一顿一顿的、帮我把动画改流畅、CSS动画性能优化、避免layout thrashing、用transform替代width/height动画、box-shadow动画优化、height auto动画的替代方案、FLIP动画技巧 | animation performance, 60fps, jank fix, layout thrashing, FLIP, compositor-only animation
version: 0.1.0
---
