---
name: gsap-web
description: GSAP 滚动动画与页面动效 | 页面往下滚才触发动画、滚到某个区域把元素钉住不动、横向滚动页面、滚动时动画跟随手指走（scrub）、首页大标题入场动画时间线、文字逐词飞入效果、用Flip做布局切换动画、GSAP跟Lenis平滑滚动配合、ScrollTrigger标记点偏移修复 | GSAP, ScrollTrigger, scroll animation, pin, scrub, Flip, Lenis, smooth scroll
version: 0.1.0
---
